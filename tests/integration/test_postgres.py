...
``make test-live``, который поднимает переносной Postgres и всегда создаёт базу заново). Здесь
проверяется именно SQL: то, что мок не проверит.

Почему база обязана быть чистой: тесты целостности журнала работают с глобальным инвариантом
(«в цепочке нет пропусков»), а тесты на подделку истории умышленно оставляют в таблице дыры.
Из-за append-only-триггеров убрать их нельзя, поэтому второй прогон по той же базе выглядел бы как
взлом — ``--fresh`` в ``make test-live`` снимает этот вопрос, а не маскирует его.
"""
