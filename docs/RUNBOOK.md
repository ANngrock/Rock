...
Распознавание — отдельная функция `parse_rate_question`, и она принципиально узкая: «добавь расход
12 евро за такси» и «запомни: рост 182» курсом не считаются, финансовый ввод по-прежнему идёт через
модель и policy engine. Проверить распознавание можно офлайн: `python evals/run_golden.py`, kind
`intent` (9 кейсов, включая «не перехватывать чужие сообщения»).
